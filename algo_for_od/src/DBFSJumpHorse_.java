...
给定 m 行 n 列的棋盘（网格图），棋盘上只有棋子象棋中的棋子“马”，并且每个棋子有等级之分，等级为 k 的马可以跳 1~k 步（走的方式与象棋中“马”的
* 规则一样，不可以超出棋盘位置），问是否能将所有马跳到同一位置，如果存在，输出最少需要的总步数（每匹马的步数相加），不存在则输出-1。
...
接下来输入 m 行 n 列的网格图棋盘，如果第 i 行，第 j 列的元素为 "." ，代表此格点没有棋子，如果为数字 k（1 ≤ k ≤ 9），代表此格点存在等级为
*  k 的“马”
...
public class DBFSJumpHorse_ {
    ...
    static int[][] stepMap;
    // 记录所有马都可达的公共位置坐标 用二维坐标一维化的方式存储 之后reach还要与别人取交集
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < m; i++) {
            ...
            // 初始时假设所有位置都是各个马可达的 为什么初始时设立成这样 因为reach是通过一步步与其他元素取交集的方式得到最终的结果 所以初始
            // 时必须是全体
            for (int j = 0; j < n; j++) {
                reach.add(i * n + j);
            }
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                ...
                if (map[i][j] != '.') {
                    // 马的等级
                    int k = map[i][j] - '0';
                    // 对该马进行BFS走日 每轮bfs的最后reach就会取交集 更新一次
                    bfs(i, j, k);
                }
            }
        }
